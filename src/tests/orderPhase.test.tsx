...
test("Toppings heading is not in summary page if toppings are not ordered", async () => {
  const user = userEvent.setup();
  // render App
  render(<App />);
  ...
  // check summary: toppings header should not be there
  const summaryHeading = screen.getByRole("heading", { name: "Order Summary" });
  expect(summaryHeading).toBeInTheDocument();

  const toppingsTotal = screen.queryByRole("heading", { name: /toppings/i });
  expect(toppingsTotal).toBeNull();
});

// Other possible test: check toppings, uncheck them and the check they don't show on the page
